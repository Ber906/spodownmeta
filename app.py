...
import subprocess, json, time, os, zipfile, shutil, logging, mimetypes, requests, base64
...
SPOTIFY_CLIENT_ID = "974318a580f84bdf90adb2b9b7f2b660"
SPOTIFY_CLIENT_SECRET = "ca3435eb9422482f846874ea47a23643"
                ...
                if process.poll() is not None:
                    ...
                    if session_id in download_states and download_states[session_id]["downloaded_size"] > 0:
                        ...
                        try:
                            ...
                            # Import at local scope to avoid circular import
                            ...
                            # Get the download ID from the session
                            download_id = download_states[session_id]["download_id"]
                            
                            # Make sure we update the status to "Completed"
                            JsonDB.update_download(download_id, {
                                "track_name": track_name,
                                "status": "Completed"
                            })
                            
                            # Double check by updating any pending downloads for this session
                            from models import User
                            downloads = User.get_by_id(download_states[session_id].get("user_id")).get_downloads()
                            for download in downloads:
                                if download.get("status") == "Pending" and download.get("spotify_url") in download_states[session_id].get("url", ""):
                                    JsonDB.update_download(download["id"], {
                                        "status": "Completed"
                                    })
...
def start_download():
    try:
        ...
        mode = request.json.get("mode", "url")  # Default to URL mode
        ...
        download_states[session_id] = {
            ...
            "user_id": current_user.id,  # Store the user ID for later status updates
            "url": url,  # Store the URL for later reference
            "completed": False,  # Flag to mark when process is fully complete
            "mode": mode  # Store the download mode
        }

        # Choose command based on mode
        if mode == "search":
            # For search mode, use spotdl directly
            ...
            # For URL mode, use the same spotdl approach but log it differently
            process = subprocess.Popen(
                ["python3", '-m', 'spotdl', url, '--output', download_folder],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
...
def get_spotify_token():
    """Get a new Spotify API token or use cached one"""
    global spotify_token, spotify_token_expiry
    
    current_time = time.time()
    if spotify_token and current_time < spotify_token_expiry:
    ...
    try:
        ...
        # Set expiry to 50 minutes (tokens last 60 minutes, we use 50 to be safe)
        spotify_token_expiry = current_time + 3000
...
def search_spotify(query, search_type="track", limit=12):
    """Search Spotify API for tracks, albums, or playlists"""
    token = get_spotify_token()
    if not token:
        return {"error": "Could not authenticate with Spotify API"}
...
def spotify_search():
    ...
    result = search_spotify(query, search_type)
    ...
    if search_type == 'track' and 'tracks' in result:
